...
function load_candidate_wp_admin_style($hook) {
    //echo $hook;
    if ($hook == 'toplevel_page_civil_post_menu') {
        wp_enqueue_script( 'post-js', plugin_dir_url( __FILE__ ) . '/assets/js/profile.js' , array(), '1.0', true );
    }
    ...
    if ($hook == 'all-post_page_candiate_post_page' OR $hook == 'tin-nhanh_page_candiate_post_page') {
...
{
    add_menu_page(__('Curriculum Vitae'), __('Curriculum Vitae'), 'candidate', 'civil_post_menu', 'civil_page', plugins_url('/images/cv.png', __FILE__));
...
require_once( CANDIDATE__PLUGIN_DIR . 'civil-page.php' );
